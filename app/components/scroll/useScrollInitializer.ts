...
export function useScrollInitializer({
  containerRef,
  wrapperRef,
  sectionsRef,
  navigateToPanel,
  ...
  useEffect(() => {
    if (!containerRef.current || !wrapperRef.current || !sectionsRef.current)
      return;
    ...
    function updateDimensions() {
      // Dimensions now handled by CSS - no GSAP dimension locking
      // Sections are 100% responsive via Tailwind classes and clamp()
    }
