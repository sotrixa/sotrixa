...
export default function Burger({ openMenu, closeMenu, isMenuOpen, isScrolled }: BurgerProps) {
	const burger = {
		closed: {
			rotate: 0,
		},
			...
			>
				...
				<div className={'burgerMenu ' + styles.burgerInner}>
					...
					<motion.div
						...
						animate={isMenuOpen ? 'opened' : 'closed'}
						style={{ position: 'absolute', width: '50px', height: '50px', top: '-15px' }}
					>
						<Image
							src='/menu-close.svg'
							alt='Close'
					...
					<motion.p
						...
						animate={isMenuOpen ? 'opened' : 'closed'}
						style={{ color: isScrolled ? 'black' : 'white', margin: 0, fontWeight: 500, fontSize: '14px' }}
					>
						MENU
					</motion.p>
				</div>
