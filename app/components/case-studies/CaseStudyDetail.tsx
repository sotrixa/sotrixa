...
export default function CaseStudyDetail({
  ...
  const goToNextCaseStudy = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const currentIndex = allCaseStudies.findIndex(
      (item) => item.study.title === currentStudy.title,
    );

    // Get next study (cycle back to 0 if at the end)
    const nextIndex =
      currentIndex < allCaseStudies.length - 1 ? currentIndex + 1 : 0;
  ...
  return (
    ...
    >
      ...
      <div
        className="grid grid-cols-12 gap-4 px-8"
      ...
      >
        ...
        <div className="col-span-6 relative h-full flex items-center">
          ...
          >
            ...
            <div
              ref={navRef}
              className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 mb-8 relative z-20 max-w-full"
            >
              {/* Back button */}
              <button
                onClick={onClose}
                className="text-gray-800 font-bold hover:text-[#3ecca7] transition-all duration-300 hover:-translate-x-1 group flex items-center flex-shrink-0"
              >
              ...
              <button
                ...
                className="text-black font-medium hover:text-gray-700 transition-all duration-300 hover:translate-x-1 group flex items-center flex-shrink-0"
              >
                <span className="flex items-center">
                  SEE NEXT CASE STUDY
                  <svg
            ...
            <div ref={contentRef} className="space-y-8 pb-72 max-w-full">
      ...
      <button
        onClick={onClose}
        className="sr-only"
        aria-label="Return to case studies"
      >
        Back
      </button>
    </div>
