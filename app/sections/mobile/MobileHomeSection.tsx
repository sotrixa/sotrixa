...
const MobileHome: React.FC = () => {
      ...
      try {
        ...
        if (gridRef.current) {
          ...
          tl.fromTo(
            columns,
            { autoAlpha: 0 },
            { autoAlpha: 0.7, stagger: 0.02, duration: 0.4 },
          );
        }
        ...
        if (headingRef.current) {
          ...
          tl.fromTo(
            parts,
            { y: 10, scale: 0.95 },
            { y: 0, scale: 1, stagger: 0.12, duration: 0.6 },
            0.3,
          );
        }
  ...
  return (
    <div
      id="mobile-home"
      ref={containerRef}
      className="relative min-h-screen w-full overflow-hidden flex items-center justify-center bg-[#fafafa]"
    >
      <div
        ref={bodyRef}
        className="relative z-10 flex flex-col items-center text-center px-4 sm:px-6 md:px-8 py-12 sm:py-16 md:py-20 max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto"
      >
        <h1
          ref={headingRef}
          className="text-[2rem] xs:text-[2.2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[3.5rem] font-bold mb-4 sm:mb-6 md:mb-8 text-black text-left"
          style={{ lineHeight: 1 }}
        >
          <span className="inline">We are a </span>
          <span className="inline text-[#53EBDD] px-2 py-1 sm:px-3 sm:py-2 rounded-lg mr-1">
            strategy
          </span>
          <span className="inline"> lab for </span>
          <span className="inline text-[#DD53EB] px-2 py-1 sm:px-3 sm:py-2 rounded-lg mr-1">
            visionary
          </span>
          <span className="inline"> </span>
          <span className="inline text-[#EBDD53] px-2 py-1 sm:px-3 sm:py-2 rounded-lg">
            thinkers
          </span>
        </h1>
        ...
        <p
          ref={paragraphRef}
          className="text-sm sm:text-base md:text-base lg:text-lg mb-6 sm:mb-8 md:mb-10 text-black max-w-full sm:max-w-md md:max-w-lg"
        >
        ...
        <div
          ref={buttonsRef}
          className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mb-4 w-full max-w-md"
        >
          <button
          ...
          <button
            ...
            onClick={() => {
              ...
              const caseStudiesSection = document.getElementById(
                "mobile-case-studies",
              );
      ...
      <div
        ...
        className="absolute inset-0 w-full h-full z-0 pointer-events-none opacity-80 sm:opacity-90 bg-white"
      >
        <video
        ...
        >
          <source
            src="/video/Sotrixa Home Page Animation.mp4"
            type="video/mp4"
          />
      ...
      <div
        ref={gridRef}
        className="absolute inset-0 z-1 opacity-5 sm:opacity-10 pointer-events-none"
      >
        <div className="flex h-full">
          {windowWidth > 0 &&
            [
              ...Array(
                windowWidth < 600 ? 8 : windowWidth < 900 ? 12 : 16,
              ).keys(),
            ].map((_, index) => (
              <div
                key={"column_" + index}
                className="grid-column flex-1 h-full"
              >
                {getBlocks()}
              </div>
            ))}
